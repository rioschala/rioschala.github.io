...
<Tabs>
  <TabItem label="Python" icon="seti:python">
    Python offers a simple syntax where a variable can store a value directly from an input and the declaration of the variable can happen in the same line where it's assigned a value. Once you master this, you can move on to [Object Oriented Programming](/posts/OOP/firstClasssOOP/). To achieve so, you need to use the command `input`. This function receives by default a string type. Hence, that's why there is no need to add `str` before it. For the other data types, it's mandatory to use the corresponding function to convert them.
